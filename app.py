...
# TAB_LABELS = ["Overall Records", "Individual Records"]
# COLORS = {
#     "primary": "green",
#     "missing": "crimson",
#     "proxy": "orange"
...
#         dhc = DataHeathCheck(uploaded_file)
#         active_tab = st.radio("Select a page", TAB_LABELS)
#
#         if active_tab == "Overall Records":
#             display_overall_records(dhc)
#         elif active_tab == "Individual Records":
#             display_individual_records(dhc)
#     else:
#         st.info("Please upload an Excel File to proceed")
#
#
# # Display overall records
# def display_overall_records(dhc):
#     st.title("Overall Records Visualization")
#
#     data_with_health_check = dhc.calculate_health_check()
#     comparison_data = dhc.create_comparison_df()
#     _, processed_data = dhc.get_data()
#
#     display_average_data_pie(data_with_health_check)
#     display_null_value_comparison(comparison_data)
#     display_categorical_distribution(processed_data)
#
#
# # Display individual records
# def display_individual_records(dhc):
...
#     display_single_data(selected_data.iloc[0])
#
#
# # Display average data pie chart
# def display_average_data_pie(data_with_health_check):
#     labels = ["primary_data_average_count", "missing_data_average_count", "proxy_data_average_count"]
#     values = [data_with_health_check["primary_data_count"].mean(),
#               data_with_health_check["missing_data_count"].mean(),
#               data_with_health_check["proxy_data_count"].mean()]
#     fig = px.pie(
#         values=values,
...
#
#
# # Display null value comparison bar chart
# def display_null_value_comparison(processed_data):
#     fig = px.bar(
#         processed_data,
#         orientation="h",
#         labels={'index': 'Columns'},
...
#     st.title("Null Value Comparison of Original Data vs Processed Data")
#     st.plotly_chart(fig, use_container_width=True)
#
#
# # Display categorical distribution
# def display_categorical_distribution(processed_data):
#     cat_cols = ['Brand', 'Item Type', 'COP',
#                 'Certification', 'material_1_name',
#                 'material_2_name', 'material_3_name',
#                 'Location of origin', 'Location of destination', 'Transport mode']
#     for col in cat_cols:
#         cat_col_counts = processed_data[col].value_counts()
#         count_bar = px.bar(x=cat_col_counts.index, y=cat_col_counts.values)
#         count_bar.update_layout(xaxis_title=str(col), yaxis_title="Count")
#         st.plotly_chart(count_bar, use_container_width=True)
#
#
# # Display individual data visualization
# def display_single_data(data):
#     labels = ["primary_data_count", "missing_data_count", "proxy_data_count"]
#     values = [data[label] for label in labels]
#     fig = px.pie(values=values, names=labels, color=labels, color_discrete_map=COLORS)
#     st.plotly_chart(fig, use_container_width=True)
#
#     for data_type in ["primary", "proxy", "missing"]:
#         columns = ", ".join(data[f"{data_type}_data_columns"])
#         st.markdown(
#             f'<div style="background-color: {COLORS[data_type]}; padding: 10px; border-radius: 10px;">'
#             f'<h3 style="color: white;"> {data_type.capitalize()} Data Columns</h3>'
#             f'<p style="color: white;">{columns}</p>'
#             '</div>',
#             unsafe_allow_html=True
#         )
#
